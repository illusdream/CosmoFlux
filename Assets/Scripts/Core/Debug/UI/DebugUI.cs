...
namespace ilsFramework.Core
{
    ...
    public class DebugUI : UIPanel
    {
        ...
        [AutoUIElement("Panel/InputField")] private TMP_InputField inputField;

        private Queue<GameObject> LogMessageList;

        private GameObject LogMessageShow;

        [AutoUIElement("Panel/Scroll View")] private ScrollRect scrollRect;
        ...
        {
            var logMessage = GetTextBeforeFirstNewline(log);
            ...
            string GetTextBeforeFirstNewline(string input)
            {
                ...
                var newlineIndex = input.IndexOfAny(new[] { '\r', '\n' });
        ...
        {
            if (!DebugManager.Instance.TryExecuteCommand(inputField.text)) inputField.text.LogSelf();
            inputField.text = "";
        }
