...
namespace ilsFramework.Core
{
    ...
    public class ConfigManager : ManagerSingleton<ConfigManager>
    {
        ...
        public override IEnumerator OnInit()
        {
            ...
            //加载所有Config
            //var assetHandler = Asset.
            foreach (var iEnumerator in Asset.LoadResourceByTagAsync<ScriptableObject>(AssetManager.DefaultPackageName,"Config",LoadResourcePriority.Default))
            {
                yield return iEnumerator;
                if (iEnumerator is LoadResourceOperation<ScriptableObject> operation)
                {
                    var asset = operation.GetResult().AssetObject;
                    _configs.Add(asset.GetType(), asset);
                }
