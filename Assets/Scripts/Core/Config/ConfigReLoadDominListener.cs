...
namespace ilsFramework.Core
{
    ...
    public class ConfigReLoadDominListener
    {
        ...
        private static void Listener_ReloadDomin()
        {
            ...
            var configsUsed = GetUsedConfigs();
            CheckRootFolderVilad(configsRootPath);

            var rootValue = new ConfigFileChecker { IsFolder = true, Old = true, New = false };

            var HasCurrectFiles = new FileTrie<ConfigFileChecker>(configsRootPath, rootValue, StartBuildChecker);
            ;

            //构建文件树，再根据文件树查询
            var guids = AssetDatabase.FindAssets("*", new[] { configsRootPath });
            var rootPath = configsRootPath + '/';
            ...
            foreach (var guid in guids)
            {
                ...
                var cPath = configsRootPath + pathWithoutExtension.Substring(rootPath.Length);
                //
                //添加到文件树
                ...
                if (!AssetDatabase.IsValidFolder(pathWithoutExtension))
                {
                    ...
                    //存在该Config
                    if (file != null && configsUsed.TryGetValue(file.GetType(), out var targetInfo))
                    {
            ...
            foreach (var checker in HasCurrectFiles)
                ...
                {
                    var ParentFolderLength = checker.GetPath().Length - checker.Name.Length - 1;

                    if (checker.value.IsFolder) AssetDatabase.CreateFolder(checker.GetPath().Substring(0, ParentFolderLength), checker.Name);
                }

            //遍历添加部分，查找哪些Config需要修改位置或者新增
            foreach (var configInfo in configsUsed.Values)
            {
                var targetPath = $"{configsRootPath}/{configInfo.OriginalPath}.asset";
        ...
        {
            if (!AssetDatabase.IsValidFolder(path.ToString())) AssetDatabase.CreateFolder(path.GetFileParentPath(), path.GetFileName());
        }

        private static Dictionary<Type, ConfigTargetInfo> GetUsedConfigs()
        {
            var result = new Dictionary<Type, ConfigTargetInfo>();
            var types = Assembly.GetExecutingAssembly().GetTypes();
            //找出对应需要遍历程序集
            foreach (var type in types)
            {
                var attribute = type.GetCustomAttribute<AutoBuildOrLoadConfig>();
                if (attribute != null)
                {
                    var cPath = $"{configsRootPath}{attribute.ConfigTargetPath}";
                    var configInfo = new ConfigTargetInfo
                    {
        ...
        private static void CleanNoUsedFolders()
        {
            ...
            var stack = new Stack<FileTrieNode<ConfigFileChecker>>();
            foreach (var trieNode in HasCurrectFiles) stack.Push(trieNode);
            var HasDeletedFiles = new HashSet<string>();
            foreach (var trieNode in stack)
                if (trieNode.value.IsFolder)
                {
                    ...
                    else
                    {
                        ...
                        foreach (var child in trieNode.Children.Values) shouldRemove &= HasDeletedFiles.Contains(child.GetPath());
        ...
        private static ConfigFileChecker StartBuildChecker()
        {
            return new ConfigFileChecker
            {
                IsFolder = true,
