...
namespace ilsFramework.Core
{
    public partial class TimerManager : ManagerSingleton<TimerManager>
    {
        ...
        private readonly LinkedList<Timer> timerList = new();
        ...
        public override void OnUpdate()
        {
            ...
            if (timerList.Count > 0)
            {
                ...
                while (currentNode != null)
                    ...
                    {
                        currentNode.Value.Update(Time.deltaTime,Time.unscaledDeltaTime);
                        currentNode = currentNode.Next;
                    }
            }
        }
        ...
        /// <param name="timerType">是否以一次Update更新为单位的计时</param>
        ...
        public Timer RegisterTimer(float cycleTime, int executingTimes, float delayTime = 0, ETimerType timerType = ETimerType.TimeScale, Action<Timer> onStart = null, Action<Timer> onCompleted = null, Action<Timer> onFinish = null, Action<Timer> onCycling = null)
        {
            int id =timerIDCounter;
            timerIDCounter++;
            Timer result = new Timer(id, cycleTime, delayTime, executingTimes, timerType, onStart, onCompleted, onFinish, onCycling);
        ...
        /// <param name="onFinish">完成全部计时时要做什么</param>
        /// <param name="onCycling">在计时过程中的每一帧要做什么</param>
        public void GetOrResetTimer(ref Timer timer, float cycleTime, int executingTimes, float delayTime = 0, ETimerType timerType = ETimerType.TimeScale, Action<Timer> onStart = null, Action<Timer> onCompleted = null, Action<Timer> onFinish = null, Action<Timer> onCycling = null)
        {
            //取消掉前一个计时器
            if (timer is not null)
                RemoveTimer(timer);
            timer = RegisterTimer(cycleTime, executingTimes, delayTime, timerType, onStart, onCompleted, onFinish, onCycling);
        }
        ...
        {
            if (timers.TryGetValue(id, out var timer)) timer.IsFinish = true;
        }
    }
}