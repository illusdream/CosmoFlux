Shader "Custom/Wire Frame Transparent"
{
    Properties
    {
       [MainTexture] _MainTex ("Base (RGB)", 2D) = "white" {}
        ...
        _Alpha ("Transparency", Range(0.0, 1.0)) = 0.5 // 新增透明度控制
    }
    CustomEditor "EdgeCustomShader"
    SubShader
    {
        Tags { 
            "Queue"="Transparent" // 修改渲染队列为透明
            "RenderType"="Transparent" // 修改渲染类型为透明
            "IgnoreProjector"="True" // 忽略投影器，避免透明物体问题
        }
        LOD 300
        ...
        Pass
        {
            ...
            fixed4 frag(g2f i) : SV_Target {
                ...
                // 应用透明度
                baseColor.a *= _Alpha;
                _WireFrameColor.a *= _Alpha;

                if(minBary <= _WireframeVal)
                {
                    // Edge pixel: blend wireframe color with base color
                    // 使用线框颜色，但保持透明度
                    return fixed4(_WireFrameColor.rgb, _WireFrameColor.a);
                }
                else
                {
                    // Non-edge pixel: return base color with transparency
                    return baseColor;
                }
            }
    ...
    FallBack "Transparent/VertexLit" // 添加一个透明备用着色器
}